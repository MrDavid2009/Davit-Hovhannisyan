...
import { User, Order, ChatMessage, Notification, PrintFile, FileFormatGroup, PaymentStatus, OrderStatus, DatabaseState, Service } from '../types';
...
export function Dashboard({ user, onLogout, database, onUpdateDatabase, onDeleteAccount }: DashboardProps) {
  ...
  const handleTouchEnd = (e: React.TouchEvent) => {
    ...
    if (Math.abs(diffX) > Math.abs(diffY) && Math.abs(diffX) > 55) {
      ...
      if (
        ...
        target.closest('.drag-drop-zone') || /* don't swipe while dragging/selecting files */
        target.closest('canvas')
      ) {
        return;
      }

      const tabs: Array<'upload' | 'orders' | 'chat' | 'profile' | 'contacts' | 'services'> = [
        'upload', 'orders', 'chat', 'profile', 'contacts', 'services'
      ];
  ...
  return (
    <div id="client-dashboard-root" onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd} className="liquid-glass-bg h-screen text-slate-800 dark:text-slate-100 flex flex-col transition-colors duration-300 relative p-1.5 sm:p-3 md:p-0 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col md:flex-row min-h-0 w-full max-w-7xl mx-auto glass-panel md:bg-transparent md:border-0 md:backdrop-blur-none md:shadow-none overflow-hidden rounded-3xl md:rounded-none relative z-10">
        ...
        {/* LEFT NAVIGATION COLUMN - Responsive Sidebar */}
        ...
        {/* Nav Links */}
        <nav className="flex md:flex-col flex-1 gap-2 md:gap-2 justify-around md:justify-start w-full relative overflow-x-auto md:overflow-visible scrollbar-none whitespace-nowrap">
          <button
          ...
          >
            ...
            <div className={`glass-icon-capsule glass-icon-violet w-9 h-9 shrink-0 relative ${activeTab === 'upload' ? 'glass-icon-active' : ''}`}>
          ...
          >
            ...
            <div className={`glass-icon-capsule glass-icon-blue w-9 h-9 shrink-0 relative ${activeTab === 'orders' ? 'glass-icon-active' : ''}`}>
          ...
          >
            ...
            <div className={`glass-icon-capsule glass-icon-pink w-9 h-9 shrink-0 relative ${activeTab === 'profile' ? 'glass-icon-active' : ''}`}>
          ...
          >
            ...
            <div className={`glass-icon-capsule glass-icon-blue w-9 h-9 shrink-0 relative ${activeTab === 'contacts' ? 'glass-icon-active' : ''}`}>
          ...
          >
            ...
            <div className={`glass-icon-capsule glass-icon-orange w-9 h-9 shrink-0 relative ${activeTab === 'services' ? 'glass-icon-active' : ''}`}>
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-slate-50/40 dark:bg-slate-950/50 backdrop-blur-md relative z-10">
        ...
        {/* WORKSPACE SECTIONS */}
        <div className={`flex-1 ${activeTab === 'chat' ? 'p-1.5 sm:p-3 md:p-8 overflow-hidden' : 'p-4 md:p-8 space-y-6 overflow-y-auto md:overflow-hidden'} max-w-6xl w-full mx-auto min-h-0 flex flex-col`}>
          {user.promoCode && (
            <motion.div 
          ...
          <AnimatePresence mode="wait">
                    ...
                    <div className="flex gap-2">
                      ...
                      {!appliedPromo ? (
                        <button type="button" onClick={handleApplyPromo}
                          className="btn-glass-3d px-3.5 py-1.5 text-xs">
                          Применить
                        </button>
                      ) : (
                        <button type="button" onClick={handleRemovePromo}
                          className="px-3.5 py-1.5 bg-rose-500/20 hover:bg-rose-500/30 border border-rose-400/40 text-rose-300 font-black text-xs rounded-xl cursor-pointer transition-all">
                          Сбросить
                        </button>
                      )}
                    </div>
                  ...
                  <button
                    ...
                    className={`w-full flex items-center justify-center gap-2 transition-all ${
                      uploadedFiles.length > 0 && isWorkingHours() && !uploadedFiles.some(f => !f.url)
                        ? 'btn-glass-3d'
                        : 'bg-white/10 text-white/30 cursor-not-allowed py-3.5 px-4 rounded-2xl text-xs font-black'
                    }`}
                  >
                          ...
                          <div className="flex gap-2 w-full sm:w-auto">
                            ...
                            {ord.paymentStatus === 'unpaid' && (
                              <button
                                ...
                                className="btn-glass-3d flex-1 sm:flex-initial"
                              >
            ...
            {activeTab === 'chat' && (
              <motion.div
                ...
                exit={{ opacity: 0, y: -15 }}
                transition={{ duration: 0.25, ease: 'easeOut' }}
                className="glass-panel rounded-3xl flex flex-col h-full sm:h-[550px] md:h-full md:flex-1 min-h-0 md:min-h-0 overflow-hidden transition-all duration-300 w-full flex-1"
              >
              ...
              <form onSubmit={handleSendMessage} className="p-4 border-t border-white/10 glass-panel flex gap-2">
                ...
                <button
                  ...
                  className={chatInput.trim() ? "btn-glass-3d shrink-0" : "bg-slate-200 dark:bg-slate-850 text-slate-400 dark:text-slate-600 py-3 px-4.5 rounded-xl font-bold text-xs flex items-center justify-center cursor-not-allowed shrink-0"}
                >
